...
|---|---|
|2025-01-26 17:11:46.579244|2.0.27|
...
| prompt_token_throughput | 5653 |
| completion_token_count_mean | 18 |
| completion_token_throughput | 21 |
| transactions_per_minute | 98 |
| price_per_txn | 0.000206 |
| price_per_token | 0.00000006 |
| latency p50, p95, p99 | 0.71, 0.71, 0.71 |
...
|-----|-----|-----|-----|-----|-----|-----|-----|
| 0% | 0.0 | 27.9 | 3.78 | 0.0 | 40270.38 | 4154.87 | 46068.0 |
| 25% | 26.3 | 27.95 | 3.79 | 87.72 | 40419.85 | 4543.35 | 46068.0 |
| 50% | 26.89 | 27.96 | 3.79 | 90.78 | 40579.65 | 4793.04 | 46068.0 |
| 75% | 27.02 | 27.99 | 3.8 | 91.26 | 40829.34 | 4952.83 | 46068.0 |
| 100% | 27.21 | 27.99 | 3.8 | 92.43 | 41217.82 | 5102.31 | 46068.0 |
...
|-----|-----|-----|-----|-----|
| llama3-8b-instruct | g5.2xlarge | nan | 102.96 | 0.034663 |
...
|`payload_en_1000-2000.jsonl`|`g5.2xlarge`|The best option for staying within a latency budget of `2 seconds` on a `g5.2xlarge` for the `payload_en_1000-2000.jsonl` dataset is a `concurrency level of 1`. A concurrency level of 1 achieves an `median latency of 0.58 seconds`, for an `average prompt size of 1566 tokens` and `completion size of 19 tokens` with `124 transactions/minute`.|
|`payload_en_2000-3000.jsonl`|`g5.2xlarge`|The best option for staying within a latency budget of `2 seconds` on a `g5.2xlarge` for the `payload_en_2000-3000.jsonl` dataset is a `concurrency level of 1`. A concurrency level of 1 achieves an `median latency of 0.65 seconds`, for an `average prompt size of 2536 tokens` and `completion size of 18 tokens` with `118 transactions/minute`.|
|`payload_en_3000-3840.jsonl`|`g5.2xlarge`|The best option for staying within a latency budget of `2 seconds` on a `g5.2xlarge` for the `payload_en_3000-3840.jsonl` dataset is a `concurrency level of 1`. A concurrency level of 1 achieves an `median latency of 0.71 seconds`, for an `average prompt size of 3424 tokens` and `completion size of 18 tokens` with `98 transactions/minute`.|
|`payload_en_500-1000.jsonl`|`g5.2xlarge`|The best option for staying within a latency budget of `2 seconds` on a `g5.2xlarge` for the `payload_en_500-1000.jsonl` dataset is a `concurrency level of 1`. A concurrency level of 1 achieves an `median latency of 0.72 seconds`, for an `average prompt size of 837 tokens` and `completion size of 27 tokens` with `88 transactions/minute`.|
