...
def _pack_dataset(dataset, chunk_length=2048):
...
def _prepare_dataset(tokenizer, dataset):
    ...
    lm_dataset = _pack_dataset(dataset, chunk_length=2048) 
    return lm_dataset


def _training_function(script_args, training_args):
    tokenizer = AutoTokenizer.from_pretrained(script_args.model_id)

    # Load dataset from the hub and prepare it for training.
    dataset = load_dataset("databricks/databricks-dolly-15k", split="train")
    dataset = _prepare_dataset(tokenizer, dataset)
...
def main():
    ...
    _training_function(script_args, training_args)
